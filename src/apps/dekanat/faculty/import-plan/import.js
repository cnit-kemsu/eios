...
export default async function importPlan(self, xml, type) {
    try {      
        ...
        // определяем формат файла с планом и выбираем соотвествующий генератор
        ...
        // считываем данные из файла с планом
            ...
            self.setState({
                message: (
                    <div>
                        ...
                        <br />
                        <a href='https://xiais.kemsu.ru/dekanat/plan/gost/addplan.htm'>Перейти к созданию нового стандарта</a>
                        <span style={{ margin: "0px 4px" }}>|</span>
                        <a href={`https://xiais.kemsu.ru/dekanat/plan/gost/dochoiceplan.htm?in_id=${planId}`}>Перейти к редактированию текущего стандарта</a>
        ...
        /*console.log(planRows, blocks)
        return*/

        response = await fetchApi("glossary/dictionaries/dic_education/missing-item-titles", {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
            },
        ...
        if (!response.ok) {
            self.setState({ message: <ErrorOutput err={await response.json()} />, messageType: "error", importing: false, forcedImport: false })
            return
        }

        let missingDisciplines = (await response.json()).missingItems
        ...
        planRows = (Object.keys(parents).map(function (key) {
            return {
                code: key,
        ...
        for (const { semesters = [] } of planRows) {
        ...
        if (realHourFlag && !self.state.forcedImport) {
            self.setState({ needConfirmImport: true, message: 'План содержит часы/ЗЕТы значения которых не являются целым.', messageType: 'warning' })
            return
        }

        /*const reqPlanData = {
            study_term: planInfo.years, //maxCourseNum,//maxCourseByGraphics,//self.state.plan.STUDY_TERM,//planInfo.years,
            learn_start_date: reverseDate(self.startDateRef.current.value),
            learn_end_date: reverseDate(self.endDateRef.current.value),
            plan_id: +planId,
        ...
        response = await fetchApi('dekanat/plans/importer', {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
            },
        ...
        let newPlanId = (await response.json()).result


        response = await fetchApi("dekanat/plans/importer/graphics", {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
            },
        ...
        response = await fetchApi("dekanat/plans/importer/plan-rows", {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
            },
        ...
        window.location = `https://xiais.kemsu.ru/dekanat/plan/work/dochoiceplan.htm?x=7&y=10&in_id=${newPlanId}`
